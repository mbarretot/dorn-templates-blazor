...
public class ThemeFamilyStylesTests
{
    ...
    private static string WebProjectRoot()
    {
        ...
        while (current is not null && !File.Exists(Path.Combine(current.FullName, "CleanArchBlazorWasm.slnx")))
        {
        ...
        return Path.Combine(current.FullName, "src", "CleanArchBlazorWasm.Web");
    }
}
